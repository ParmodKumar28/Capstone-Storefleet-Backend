...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    const{page, keyword, category, ...filters} = req.query;
    ...
    if(keyword){
      query.name = {$regex: new RegExp(keyword), $options: 'i'};
    }

    if(filters.rating)
    {
      query.rating = {
        $gte: filters.rating.gte || 0,
        $lte: filters.rating.lte || 5,
      };
    }
    ...
    if(filters.price)
    {
      query.price = {
        ...
        $lte: filters.price.lte || Infinity,
      };
    }
    ...
    {
      return next(new ErrorHandler(400, "No produt found"));
    }
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    // Authorizing user before deleting the review.
    ...
    {
      return next(new ErrorHandler(400, "you are not allowed to delete another review."));
    }
    
    const updatedProduct = await findProductRepo(productId);
    // Maintaining average ratings of the product after deleting review.
    let avgRating = 0;
    updatedProduct.reviews.forEach((rev) => {
      avgRating += rev.rating;
    });
    const updatedRatingOfProduct = avgRating / updatedProduct.reviews.length;
    updatedProduct.rating = updatedRatingOfProduct;
